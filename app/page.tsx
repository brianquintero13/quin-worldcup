                                        ...
                                        <div key={drafter}>
                                            ...
                                            <div className="grid grid-cols-2 gap-1 sm:gap-1.5 pl-2">
                                                {picks.filter(p => p.drafter === drafter).map((pick, pIdx) => (
                                                    <div key={pIdx} className="bg-black/60 border border-white/20 rounded-md px-1.5 sm:px-2 py-1 sm:py-1.5 flex items-center text-[9px] sm:text-xs shadow-md min-w-0">
                                            ...
                                            <div key={group}>
                                                ...
                                                <div className="grid grid-cols-2 sm:grid-cols-4 gap-1.5 sm:gap-3">
                                                    {groupTeams.sort().map(team => {
                                                        ...
                                                        return (
                                                            <div key={team} className={`border rounded-lg p-1.5 sm:p-2 flex flex-col justify-center items-center text-center transition-all min-w-0 shadow-lg ${drafter ? 'bg-black/90 border-white/5 opacity-70' : 'bg-black/60 border-white/30 hover:border-white/50 hover:bg-black/80'}`}>
                                                                <div className="mb-0.5 sm:mb-1"><FlagIcon teamName={team}/></div>
                                                                <span className={`text-[9px] sm:text-[11px] truncate block w-full drop-shadow-[0_2px_2px_rgba(0,0,0,1)] ${drafter ? 'line-through text-slate-400 font-bold' : 'font-black text-slate-100'}`}>{team}</span>
                                                                <span className="text-[7px] sm:text-[9px] text-sky-400 font-mono mt-0.5 sm:mt-1 h-2 drop-shadow-md font-black">{drafter || ''}</span>
                                                            </div>
                                                        );
                                                    })}
                                                </div>
