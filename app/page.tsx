            ...
            <div className="relative z-10 p-3 sm:p-5">
                ...
                {selectedManager && (
                    <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-3 sm:p-5 overflow-y-auto backdrop-blur-sm" onClick={() => setSelectedManager(null)}>
                        <div className="bg-black/80 backdrop-blur-3xl border border-white/10 rounded-xl max-w-4xl w-full max-h-[90vh] flex flex-col shadow-2xl" onClick={e => e.stopPropagation()}>
                                        ...
                                        <div key={team} className="bg-white/[0.02] border border-white/10 rounded-lg overflow-hidden shadow-sm">
                                            ...
                                            <div className="overflow-x-auto">
                                                <table className="w-full text-left text-xs border-collapse min-w-[500px]">
                                                    <thead>
                                                    <tr className="border-b border-white/5 text-slate-500 text-[10px] uppercase font-mono bg-black/20 tracking-widest">
                                                        ...
                                                        logs.map((log: any, i: number) => (
                                                            <tr key={i} className="hover:bg-white/5 transition">
                                                                ...
                                                                <td className="py-3 text-right pr-4">
                                                                    {log.points > 0 ? (
                                                                        <span className={`font-black text-emerald-400 text-[13px] sm:text-[15px] bg-black/40 border border-white/5 px-2.5 py-1 rounded shadow-inner ${oswald.className}`}>+{log.points}</span>
                                                                    ) : (
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-center gap-3 sm:gap-4">
                        <div className="text-center md:text-left">
                            <h1 className="text-2xl sm:text-3xl font-black tracking-tight uppercase flex items-center gap-2 justify-center md:justify-start">
                                ...
                                <span className={`text-transparent bg-clip-text bg-gradient-to-r from-emerald-400 via-fuchsia-500 to-rose-500 ${oswald.className}`}>League World Cup</span>
                            </h1>
                        </div>
                ...
                <div key={`content-${activeTab}`} className="content-animate">
                    ...
                    {activeTab === 'draft' && (
                        <div className="grid grid-cols-1 lg:grid-cols-4 gap-4 sm:gap-6 max-w-7xl mx-auto">
                            {/* Drafters Column */}
                            <div className="bg-black/40 backdrop-blur-xl rounded-xl border border-white/10 overflow-hidden flex flex-col h-[60vh] lg:h-[80vh] shadow-2xl">
                                <div className="p-3.5 border-b border-white/10 bg-black/50">
                                    <h2 className="text-xs font-mono font-bold text-slate-400 uppercase tracking-widest">Drafters & Picks</h2>
                                </div>
                                    ...
                                    {drafters.map((drafter, idx) => (
                                        <div key={drafter}>
                                            <div className="flex items-center gap-2 mb-2">
                                                ...
                                                <h3 className="font-bold text-slate-200 text-sm">{drafter}</h3>
                                            </div>
                                            <div className="grid grid-cols-2 gap-2 pl-4">
                                                {picks.filter(p => p.drafter === drafter).map((pick, pIdx) => (
                                                    <div key={pIdx} className="bg-white/5 border border-white/10 rounded px-2 py-1.5 flex items-center text-[11px] sm:text-xs shadow-sm min-w-0">
                            ...
                            {/* Groups Column */}
                            <div className="lg:col-span-2 bg-black/40 backdrop-blur-xl rounded-xl border border-white/10 overflow-hidden flex flex-col h-[70vh] lg:h-[80vh] shadow-2xl">
                                <div className="p-3.5 border-b border-white/10 bg-black/50 flex justify-between items-center gap-2">
                                    <h2 className="text-[11px] sm:text-xs font-mono font-bold text-slate-400 uppercase tracking-widest truncate">Tournament Field</h2>
                                    <input type="text" placeholder="Search..." value={draftSearch} onChange={(e) => setDraftSearch(e.target.value)} className="bg-black/50 border border-white/10 text-slate-200 rounded px-2.5 py-1 text-[11px] sm:text-xs focus:outline-none focus:border-emerald-400 w-24 sm:w-40 transition" />
                                </div>
                                <div className="overflow-y-auto p-3 sm:p-4 space-y-5">
                                    {Object.keys(groupedTeams).sort().map(group => {
                                        ...
                                        return (
                                            <div key={group}>
                                                <h3 className="text-[10px] font-mono font-bold text-slate-500 mb-2 uppercase tracking-widest">{group}</h3>
                                                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3">
                                                    {groupTeams.sort().map(team => {
                                                        const drafter = getDrafterForTeam(team);
                                                        return (
                                                            <div key={team} className={`border rounded p-2 sm:p-3 flex flex-col justify-center items-center text-center transition-all min-w-0 ${drafter ? 'bg-black/30 border-white/5 opacity-50' : 'bg-white/5 border-white/10 shadow-sm hover:border-white/20 hover:bg-white/10'}`}>
                                                                <div className="mb-1 sm:mb-1.5"><FlagIcon teamName={team} /></div>
                                                                <span className={`text-[11px] sm:text-xs break-words whitespace-normal leading-tight ${drafter ? 'line-through text-slate-500' : 'font-bold text-slate-200'}`}>{team}</span>
                            ...
                            {/* Pick Log Column */}
                            <div className="bg-black/40 backdrop-blur-xl rounded-xl border border-white/10 overflow-hidden flex flex-col h-[50vh] lg:h-[80vh] shadow-2xl">
                                <div className="p-3.5 border-b border-white/10 bg-black/50">
                                    <h2 className="text-xs font-mono font-bold text-slate-400 uppercase tracking-widest">Pick Log</h2>
                                </div>
                                <div className="overflow-y-auto p-3.5 space-y-2">
                                    {[...picks].reverse().map((pick, idx) => (
                                        <div key={idx} className="flex items-center justify-between bg-white/5 border border-white/10 p-2 sm:p-2.5 rounded min-w-0 hover:bg-white/10 transition">
                                            <div className="flex items-center gap-2 min-w-0 pr-2">
                                                <span className={`text-xs font-mono text-slate-500 shrink-0 w-5 ${oswald.className}`}>#{picks.length - idx}</span>
                                                <div className="flex items-center text-[11px] sm:text-sm font-bold text-slate-200 min-w-0">
                                                    <FlagIcon teamName={pick.team} />
                                                    <span className="break-words whitespace-normal leading-tight">{pick.team}</span>
                                                </div>
                                            </div>
                                            <span className="text-[9px] sm:text-[10px] text-slate-400 shrink-0">{pick.drafter}</span>
                                        </div>
                                    ))}
                                </div>
                            </div>
                        ...
                        <div className="max-w-7xl mx-auto space-y-5">
                            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-3 mb-5">
                                ...
                                <div className="flex items-center gap-2 w-full sm:w-auto">
                                    <label htmlFor="groupFilter" className="text-[10px] sm:text-xs font-mono text-slate-400 uppercase tracking-widest shrink-0">Filter:</label>
                                    <select
                                        id="groupFilter"
                                        value={selectedGroupFilter}
                                        onChange={(e) => setSelectedGroupFilter(e.target.value)}
                                        className="bg-black/60 backdrop-blur-md border border-white/10 text-slate-200 rounded px-3 py-1.5 text-xs focus:outline-none focus:border-emerald-400 transition shadow-lg w-full sm:w-auto"
                                    >
                            ...
                            {filteredGroupNames.length === 0 ? (
                                <div className="text-center py-8 text-slate-400 bg-black/40 backdrop-blur-xl border border-dashed border-white/20 rounded-xl text-xs">
                                ...
                                filteredGroupNames.map(group => {
                                    ...
                                    return (
                                        <div key={group} className="bg-black/40 backdrop-blur-xl border border-white/10 rounded-xl overflow-hidden shadow-2xl">
                                            <div className="bg-black/50 px-3 sm:px-4 py-2.5 sm:py-3 border-b border-white/10 flex justify-between items-center">
                                                <h3 className={`font-black text-[#fbbf24] text-xs sm:text-sm uppercase tracking-widest ${oswald.className}`}>{group}</h3>
                                                <span className="text-[9px] sm:text-[10px] text-slate-500 font-mono uppercase tracking-widest">Top 2 advance</span>
                                            </div>
                                            ...
                                            <div className="flex flex-col lg:flex-row">
                                                ...
                                                <div className="w-full lg:w-[35%] xl:w-[30%] border-b lg:border-b-0 lg:border-r border-white/10 overflow-x-auto flex bg-black/20">
                                                    <table className="w-full text-left text-[11px] sm:text-[12px] min-w-[320px]">
                                                        <thead>
                                                        <tr className="border-b border-white/10 text-slate-400 text-[9px] sm:text-[10px] uppercase font-mono bg-white/5">
                                                        ...
                                                        {groupTable.map((teamRow) => (
                                                            <tr key={teamRow.name} className="hover:bg-white/10 transition">
                                                                <td className="py-3 sm:py-5 lg:py-6 pl-3 sm:pl-4 font-bold text-slate-200">
                                                                ...
                                                                <td className="py-3 sm:py-5 lg:py-6 text-center text-slate-300 font-mono">{teamRow.mp}</td>
                                                                <td className="py-3 sm:py-5 lg:py-6 text-center text-slate-300 font-mono">{teamRow.w}</td>
                                                                <td className="py-3 sm:py-5 lg:py-6 text-center text-slate-300 font-mono">{teamRow.d}</td>
                                                                <td className="py-3 sm:py-5 lg:py-6 text-center text-slate-300 font-mono">{teamRow.l}</td>
                                                                <td className="py-3 sm:py-5 lg:py-6 text-center text-slate-300 font-mono">{teamRow.gf}</td>
                                                                <td className="py-3 sm:py-5 lg:py-6 text-center text-slate-300 font-mono">{teamRow.ga}</td>
                                                ...
                                                <div className="w-full lg:w-[65%] xl:w-[70%] p-2 sm:p-4 grid grid-cols-1 md:grid-cols-2 gap-2 sm:gap-4 content-start bg-transparent">
                                                    {groupMatches.map(m => {
                                                        ...
                                                        const homeNameColor = isHomeWin ? 'font-black text-emerald-400' : isAwayWin ? 'font-medium text-rose-400' : 'font-bold text-slate-100';
                                                        const awayNameColor = isAwayWin ? 'font-black text-emerald-400' : isHomeWin ? 'font-medium text-rose-400' : 'font-bold text-slate-100';
                                                        ...
                                                        return (
                                                            <div key={m.id} className="flex items-center justify-between p-2 sm:p-3 bg-white/5 border border-white/10 rounded-lg hover:bg-white/10 transition shadow-sm h-full">
                                                                ...
                                                                <div className="flex-1 flex flex-col items-end text-right min-w-0">
                                                                    ...
                                                                    {homeDrafter && <span className="text-[8px] sm:text-[9px] text-slate-400 font-mono mt-1 shrink-0 truncate max-w-full">{homeDrafter}</span>}
                                                                </div>

                                                                {/* Score Block */}
                                                                <div className="mx-2 sm:mx-4 flex flex-col items-center shrink-0 min-w-[55px] sm:min-w-[65px]">
                                                                    <div className="flex items-center justify-center gap-1 sm:gap-1.5 bg-black/50 px-2 sm:px-2.5 py-1 sm:py-1.5 rounded border border-white/10 w-full shadow-inner mb-0.5 sm:mb-1">
                                                                ...
                                                                <div className="flex-1 flex flex-col items-start text-left min-w-0">
                                                                    ...
                                                                    {awayDrafter && <span className="text-[8px] sm:text-[9px] text-slate-400 font-mono mt-1 shrink-0 truncate max-w-full">{awayDrafter}</span>}
                                                                </div>
                                                            </div>
                                                        )
                                                    })}
                    ...
                    {activeTab === 'standings' && (
                        <div className="max-w-6xl mx-auto space-y-4 sm:space-y-6">
                            ...
                            {/* SCORING LEGEND */}
                            <div className="bg-black/40 backdrop-blur-xl border border-white/10 rounded-xl p-3 sm:p-5 shadow-2xl hidden md:block">
                                <h3 className="text-[10px] font-mono font-bold text-slate-400 uppercase tracking-widest mb-3">Point System</h3>
                                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-4 gap-y-2 gap-x-4 text-[11px] sm:text-[12px]">
                                    <div className="space-y-1.5">
                                        <div className="flex items-center gap-2"><span className={`text-[#fbbf24] font-black w-6 text-right text-sm ${oswald.className}`}>+4</span> <span className="text-slate-300">Win Match</span></div>
                                        <div className="flex items-center gap-2"><span className={`text-[#fbbf24] font-black w-6 text-right text-sm ${oswald.className}`}>+8</span> <span className="text-slate-300">Advance</span></div>
                                        <div className="flex items-center gap-2"><span className={`text-[#fbbf24] font-black w-6 text-right text-sm ${oswald.className}`}>+20</span> <span className="text-slate-300">Win SF</span></div>
                                    </div>
                                    <div className="space-y-1.5">
                                        <div className="flex items-center gap-2"><span className={`text-[#fbbf24] font-black w-6 text-right text-sm ${oswald.className}`}>+2</span> <span className="text-slate-300">Group Draw</span></div>
                                        <div className="flex items-center gap-2"><span className={`text-[#fbbf24] font-black w-6 text-right text-sm ${oswald.className}`}>+10</span> <span className="text-slate-300">Win R32</span></div>
                                        <div className="flex items-center gap-2"><span className={`text-[#fbbf24] font-black w-6 text-right text-sm ${oswald.className}`}>+10</span> <span className="text-slate-300">Win 3rd</span></div>
                                    </div>
                                    <div className="space-y-1.5">
                                        <div className="flex items-center gap-2"><span className={`text-[#fbbf24] font-black w-6 text-right text-sm ${oswald.className}`}>+1</span> <span className="text-slate-300">Goal Scored</span></div>
                                        <div className="flex items-center gap-2"><span className={`text-[#fbbf24] font-black w-6 text-right text-sm ${oswald.className}`}>+12</span> <span className="text-slate-300">Win R16</span></div>
                                        <div className="flex items-center gap-2"><span className={`text-[#fbbf24] font-black w-6 text-right text-sm ${oswald.className}`}>+30</span> <span className="text-slate-300">Win Final</span></div>
                                    </div>
                                    <div className="space-y-1.5">
                                        <div className="flex items-center gap-2"><span className={`text-[#fbbf24] font-black w-6 text-right text-sm ${oswald.className}`}>+2</span> <span className="text-slate-300">Clean Sheet</span></div>
                                        <div className="flex items-center gap-2"><span className={`text-[#fbbf24] font-black w-6 text-right text-sm ${oswald.className}`}>+15</span> <span className="text-slate-300">Win QF</span></div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-3 gap-2 sm:gap-4 md:gap-6">
                                {overallLeaders.slice(0, 3).map((leader, i) => (
                                    <div key={leader.name} className={`backdrop-blur-xl rounded-xl flex flex-col items-center justify-center p-2 sm:p-6 text-center transition-all duration-300 ${
                                        i === 0 ? 'bg-amber-500/20 border border-amber-400/50 shadow-[0_0_30px_rgba(251,191,36,0.3)]' :
                                            i === 1 ? 'bg-slate-300/10 border border-slate-300/30 shadow-[0_0_30px_rgba(203,213,225,0.15)]' :
                                                'bg-orange-700/20 border border-orange-700/40 shadow-[0_0_30px_rgba(194,65,12,0.2)]'
                                    }`}>
                                        ...
                                        <span className="text-[8px] sm:text-[10px] text-slate-300 font-mono mb-2 sm:mb-4 uppercase tracking-widest hidden sm:block">Points</span>
                            ...
                            <div className="bg-black/40 backdrop-blur-xl border border-white/10 rounded-xl overflow-hidden shadow-2xl overflow-x-auto">
                                <table className="w-full text-left text-[12px] sm:text-[14px] border-collapse min-w-[600px] sm:min-w-[700px]">
                                    <thead>
                                    <tr className="border-b border-white/10 text-slate-400 text-[9px] sm:text-[10px] uppercase font-mono bg-black/40 tracking-widest">
                                    ...
                                    {overallLeaders.map((row, index) => (
                                        <tr key={row.name} className="hover:bg-white/10 transition">
                                            <td className={`py-3 sm:py-4 pl-4 sm:pl-6 font-bold text-slate-400 text-[13px] sm:text-[15px] ${oswald.className}`}>{index + 1}</td>
                                            ...
                                            <td className={`py-3 sm:py-4 text-center font-bold text-slate-300 text-[13px] sm:text-[15px] ${oswald.className}`}>{row.draws}</td>
                    ...
                    {activeTab === 'awards' && (
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-8 max-w-7xl mx-auto">
                            ...
                            {/* Golden Boot Card */}
                            <div className="bg-gradient-to-br from-amber-500/40 to-orange-600/40 p-[1.5px] rounded-xl shadow-2xl h-full">
                                <div className="bg-black/40 backdrop-blur-xl p-4 sm:p-7 rounded-xl h-full flex flex-col">
                                    <div className="flex items-center gap-3 sm:gap-4 mb-4 sm:mb-6 border-b border-white/10 pb-3 sm:pb-4">
                                        <div className="bg-black/40 p-2 sm:p-3 rounded-lg border border-amber-500/20 shadow-inner">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-amber-400/80 text-[9px] sm:text-xs font-mono font-bold tracking-widest uppercase mt-0.5 sm:mt-1">15% Pot • Most Goals</p>
                                        </div>
                                    </div>
                                    ...
                                    <div className="space-y-2 sm:space-y-3 flex-1">
                                        {bootLeaders.slice(0, 5).map((row, idx) => {
                                            ...
                                            return (
                                                <div key={row.name} className={`flex justify-between items-center p-2.5 sm:p-4 rounded-lg border transition-all ${idx === 0 ? 'bg-amber-500/20 border-amber-400/50 shadow-md scale-[1.02]' : 'bg-white/5 border-white/10 hover:border-white/20'}`}>
                                                    <div className="flex items-center gap-2 sm:gap-4 min-w-0">
                                                        <span className={`font-black text-lg sm:text-2xl w-5 sm:w-6 shrink-0 text-center ${idx === 0 ? 'text-amber-400' : 'text-slate-400'}`}>{idx === 0 ? '🥇' : idx === 1 ? '🥈' : idx === 2 ? '🥉' : `${idx+1}.`}</span>
                                                        <div className="flex flex-col min-w-0 pr-2">
                                                            <span className={`font-bold text-[13px] sm:text-[16px] leading-tight break-words whitespace-normal ${idx === 0 ? 'text-white' : 'text-slate-300'}`}>{row.name}</span>
                                                            <span className="text-[9px] sm:text-[11px] text-slate-400 mt-0.5 sm:mt-1 max-w-[120px] sm:max-w-[240px] truncate" title={breakdownText}>
                                                    ...
                                                    <div className="flex flex-col items-end shrink-0">
                                                        ...
                                                        <span className="text-[8px] sm:text-[10px] text-slate-400 font-mono uppercase tracking-widest mt-0.5 sm:mt-1">Goals</span>
                                                    </div>
                                                </div>
                                            )
                                        })}
                            ...
                            {/* Golden Glove Card */}
                            <div className="bg-gradient-to-br from-blue-400/40 to-blue-700/40 p-[1.5px] rounded-xl shadow-2xl h-full">
                                <div className="bg-black/40 backdrop-blur-xl p-4 sm:p-7 rounded-xl h-full flex flex-col">
                                    <div className="flex items-center gap-3 sm:gap-4 mb-4 sm:mb-6 border-b border-white/10 pb-3 sm:pb-4">
                                        <div className="bg-black/40 p-2 sm:p-3 rounded-lg border border-blue-500/20 shadow-inner">
                                            <span className="text-2xl sm:text-4xl block leading-none">🧤</span>
                                        </div>
                                        <div>
                                            ...
                                            <p className="text-blue-400/80 text-[9px] sm:text-xs font-mono font-bold tracking-widest uppercase mt-0.5 sm:mt-1">10% Pot • Clean Sheets</p>
                                        </div>
                                    </div>
                                    ...
                                    <div className="space-y-2 sm:space-y-3 flex-1">
                                        {gloveLeaders.slice(0, 5).map((row, idx) => {
                                            ...
                                            return (
                                                <div key={row.name} className={`flex justify-between items-center p-2.5 sm:p-4 rounded-lg border transition-all ${idx === 0 ? 'bg-blue-500/20 border-blue-400/50 shadow-md scale-[1.02]' : 'bg-white/5 border-white/10 hover:border-white/20'}`}>
                                                    <div className="flex items-center gap-2 sm:gap-4 min-w-0">
                                                        <span className={`font-black text-lg sm:text-2xl w-5 sm:w-6 shrink-0 text-center ${idx === 0 ? 'text-blue-400' : 'text-slate-400'}`}>{idx === 0 ? '🥇' : idx === 1 ? '🥈' : idx === 2 ? '🥉' : `${idx+1}.`}</span>
                                                        <div className="flex flex-col min-w-0 pr-2">
                                                            <span className={`font-bold text-[13px] sm:text-[16px] leading-tight break-words whitespace-normal ${idx === 0 ? 'text-white' : 'text-slate-300'}`}>{row.name}</span>
                                                            <span className="text-[9px] sm:text-[11px] text-slate-400 mt-0.5 sm:mt-1 max-w-[120px] sm:max-w-[240px] truncate" title={breakdownText}>
                                                    ...
                                                    <div className="flex flex-col items-end shrink-0">
                                                        ...
                                                        <span className="text-[8px] sm:text-[10px] text-slate-400 font-mono uppercase tracking-widest mt-0.5 sm:mt-1">Sheets</span>
                                                    </div>
                                                </div>
                                            )
                                        })}
